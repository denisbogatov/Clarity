...
class BlenderGlTF():
    ...
    def set_convert_functions(gltf):
        if bpy.app.debug_value != 100:
            ...
            # glTF and Clarity both use a native Y-up world basis.
            def convert_loc(x): return u * Vector(x)
            def convert_quat(q): return Quaternion([q[3], q[0], q[1], q[2]])
            def convert_scale(s): return Vector(s)

            def convert_matrix(m):
                matrix = Matrix([m[0::4], m[1::4], m[2::4], m[3::4]])
                matrix.translation = matrix.translation * u
                return matrix
            ...
            # Cameras and lights already use glTF's local -Z forward, +Y up basis.
