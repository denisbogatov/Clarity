...
def _is_topology_selection_conflict(item):
    ...
    # The Clarity interaction model dispatches those gestures itself, before any keymap runs, so the
...
def _is_shortest_path_conflict(item):
    ...
    # for the component pairs Clarity walks a path between, so the base binding only stands in the way.
    return (
...
def apply_clarity_selection_shortcuts(keyconfig_data):
    """Move the three selection-wide commands onto the keys Clarity uses for them.

    Industry Compatible puts them on `Ctrl A`, `Ctrl Shift A` and `Ctrl I`; Clarity uses
    ...
    for _keymap_name, _keymap_args, keymap_content in keyconfig_data:
        for item in keymap_content["items"]:
            ...
            if action == 'SELECT' and properties.get("type") == 'A':
                ...
                # Clarity deselects everything with `Alt D`, and leaves `A` to selecting.
                properties.clear()
...
def apply_clarity_modeling_shortcuts(keyconfig_data):
    """Reserve Clarity modeling chords for one direct operator each."""
...
def remove_clarity_key_conflicts(keyconfig_data):
    """Drop base bindings that occupy gestures the Clarity interaction model owns itself.
...
def load():
    ...
    remove_clarity_key_conflicts(keyconfig_data)
