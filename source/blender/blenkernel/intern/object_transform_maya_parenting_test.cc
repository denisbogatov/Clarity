...
static MayaObjectTransform *maya_transform_create(Object &object)
{
  object.transform_model = OBJECT_TRANSFORM_MAYA;
  return BKE_object_maya_transform_ensure(&object);
}

TEST(object_transform_maya_parenting, MatrixLayers)
{
  ...
  const double4x4 channel_matrix = BKE_maya_transform_channel_matrix(transform);
  ...
  expect_matrix_near(BKE_maya_transform_world_matrix(transform, parent_effect_matrix),
...
TEST(object_transform_maya_parenting, ObjectPathsUseSameMatrix)
{
  Object parent;
  Object child;
  ...
  MayaObjectTransform *transform = maya_transform_create(child);
  ...
  BKE_object_where_is_calc_mat4(&child, matrix);
  expect_matrix_near(
      double4x4(float4x4(matrix)), BKE_maya_transform_dag_local_matrix(*transform), 1.0e-6);

  MEM_SAFE_DELETE(child.maya_transform);
}

TEST(object_transform_maya_parenting, ParentAndClearKeepWorldTransform)
{
  Object first_parent;
  ...
  const double4x4 world_before = BKE_maya_transform_dag_local_matrix(*transform);
  child.parent = &first_parent;
  child.partype = PAROBJECT;
  ASSERT_TRUE(BKE_object_maya_parent_keep_transform(&child, first_parent_effect, world_before));
  ...
  ASSERT_TRUE(BKE_object_maya_parent_keep_transform(&child, second_parent_effect, world_before));
  ...
  ASSERT_TRUE(BKE_object_maya_clear_parent_keep_transform(&child, parented_world));
...
TEST(object_transform_maya_parenting, TransformChannelCopyIsDeep)
{
  Object source;
  Object destination;
  MayaObjectTransform *source_transform = maya_transform_create(source);
  source_transform->translation[0] = 8.0;
  source_transform->rotate_axis[1] = 0.25;
  source_transform->rotation_order = MAYA_ROT_ORDER_YZX;

  BKE_object_tfm_copy(&destination, &source);

  ASSERT_TRUE(BKE_object_uses_maya_transform(&destination));
  ASSERT_NE(destination.maya_transform, source.maya_transform);
  EXPECT_DOUBLE_EQ(destination.maya_transform->translation[0], 8.0);
  EXPECT_DOUBLE_EQ(destination.maya_transform->rotate_axis[1], 0.25);
  EXPECT_EQ(destination.maya_transform->rotation_order, MAYA_ROT_ORDER_YZX);

  source.maya_transform->translation[0] = -10.0;
  EXPECT_DOUBLE_EQ(destination.maya_transform->translation[0], 8.0);

  MEM_SAFE_DELETE(source.maya_transform);
  MEM_SAFE_DELETE(destination.maya_transform);
}

TEST(object_transform_maya_parenting, PivotCentersAndAxesUseMayaPrefixes)
{
  Object object;
  MayaObjectTransform *transform = maya_transform_create(object);
  ...
  const double3 rotate_center = BKE_object_maya_rotate_pivot_world_get(object, parent_effect);
  EXPECT_NEAR(rotate_center.x, 12.5, 1.0e-12);
  EXPECT_NEAR(rotate_center.y, 21.0, 1.0e-12);
  EXPECT_NEAR(rotate_center.z, 30.0, 1.0e-12);

  const double3 scale_center = BKE_object_maya_scale_pivot_world_get(object, parent_effect);
  ...
  EXPECT_TRUE(BKE_object_maya_local_axis_world_get(object, parent_effect, local_axis));
  EXPECT_TRUE(BKE_object_maya_parent_axis_world_get(object, parent_effect, parent_axis));
  EXPECT_TRUE(BKE_object_maya_gimbal_axis_world_get(object, parent_effect, gimbal_axis));
...
TEST(object_transform_maya_parenting, PivotWorldSettersPreserveMatrix)
{
  Object object;
  MayaObjectTransform *transform = maya_transform_create(object);
  transform->translation[0] = 2.0;
  ...
  const double4x4 matrix_before = BKE_maya_transform_world_matrix(*transform, parent_effect);
  const double3 target_world = {9.25, -4.5, 6.75};
  ASSERT_TRUE(
      BKE_object_maya_rotate_pivot_world_set(object, parent_effect, target_world, true));
  ASSERT_TRUE(
      BKE_object_maya_scale_pivot_world_set(object, parent_effect, target_world, true));

  expect_matrix_near(BKE_maya_transform_world_matrix(*transform, parent_effect), matrix_before);
  EXPECT_NEAR(BKE_object_maya_rotate_pivot_world_get(object, parent_effect).x,
              target_world.x,
              1.0e-11);
  EXPECT_NEAR(BKE_object_maya_rotate_pivot_world_get(object, parent_effect).y,
              target_world.y,
              1.0e-11);
  EXPECT_NEAR(BKE_object_maya_rotate_pivot_world_get(object, parent_effect).z,
              target_world.z,
              1.0e-11);
  EXPECT_NEAR(BKE_object_maya_scale_pivot_world_get(object, parent_effect).x,
              target_world.x,
              1.0e-11);
  EXPECT_NEAR(BKE_object_maya_scale_pivot_world_get(object, parent_effect).y,
              target_world.y,
              1.0e-11);
  EXPECT_NEAR(BKE_object_maya_scale_pivot_world_get(object, parent_effect).z,
              target_world.z,
              1.0e-11);
...
TEST(object_transform_maya_parenting, TransformModelConversionPreservesWorld)
{
  Object object;
  ...
  ASSERT_TRUE(BKE_object_transform_model_set(object, OBJECT_TRANSFORM_MAYA));
  ASSERT_TRUE(BKE_object_uses_maya_transform(&object));
  float maya_world[4][4];
  BKE_object_where_is_calc_mat4(&object, maya_world);
  expect_matrix_near(double4x4(float4x4(maya_world)), world_before, 1.0e-5);

  ASSERT_TRUE(BKE_object_transform_model_set(object, OBJECT_TRANSFORM_BLENDER));
  EXPECT_FALSE(BKE_object_uses_maya_transform(&object));
  EXPECT_EQ(object.maya_transform, nullptr);
...
TEST(object_transform_maya_parenting, TransformModelConversionFailureIsAtomic)
{
  Object object;
  ObjectRuntime runtime;
  object.runtime = &runtime;
  ...
  const MayaObjectTransform before = *transform;
  EXPECT_FALSE(BKE_object_transform_model_set(object, OBJECT_TRANSFORM_BLENDER));
  EXPECT_TRUE(BKE_object_uses_maya_transform(&object));
  EXPECT_EQ(memcmp(object.maya_transform, &before, sizeof(before)), 0);

  MEM_SAFE_DELETE(object.maya_transform);
}

TEST(object_transform_maya_parenting, BoneParentUsesBlenderParentEffect)
{
  Main *bmain = BKE_main_new();
  bArmature *armature = BKE_armature_add(bmain, "Maya Parent Armature");
  Bone *bone = MEM_new<Bone>(__func__);
  STRNCPY(bone->name, "Maya Parent Bone");
  bone->length = 2.0f;
  BLI_addtail(&armature->bonebase, bone);

  Object *parent = BKE_object_add_only_object(bmain, OB_ARMATURE, "Maya Parent");
  ...
  Object *child = BKE_object_add_only_object(bmain, OB_EMPTY, "Maya Child");
  child->parent = parent;
  ...
  expect_matrix_near(double4x4(float4x4(world_matrix)),
                     double4x4(float4x4(parent_effect)) *
                         BKE_maya_transform_dag_local_matrix(*transform),
                     1.0e-6);
