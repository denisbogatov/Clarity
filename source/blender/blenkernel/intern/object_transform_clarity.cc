...
{
  ClarityTransformCapabilities capabilities;
  switch (object.type) {
    case OB_MESH:
...
{
  transform = ClarityObjectTransform();
}

eEulerRotationOrders BKE_clarity_rotation_order_to_blender(const eClarityRotationOrder order)
{
  switch (order) {
...
eClarityRotationOrder BKE_clarity_rotation_order_from_blender(const eEulerRotationOrders order)
{
  switch (order) {
    case EULER_ORDER_YZX:
      return CLARITY_ROT_ORDER_YZX;
    case EULER_ORDER_ZXY:
      return CLARITY_ROT_ORDER_ZXY;
    case EULER_ORDER_XZY:
      return CLARITY_ROT_ORDER_XZY;
    case EULER_ORDER_YXZ:
      return CLARITY_ROT_ORDER_YXZ;
    case EULER_ORDER_ZYX:
      return CLARITY_ROT_ORDER_ZYX;
    case EULER_ORDER_XYZ:
    default:
      return CLARITY_ROT_ORDER_XYZ;
  }
}
...
static double4x4 rotate_axis_matrix(const ClarityObjectTransform &transform)
{
  return math::from_rotation<double4x4>(BKE_clarity_rotate_axis_quaternion(transform));
}
...
{
  return double4x4(transform.offset_parent_matrix) * BKE_clarity_transform_channel_matrix(transform);
}
...
static math::EulerOrder clarity_math_rotation_order(const eClarityRotationOrder order)
{
  switch (BKE_clarity_rotation_order_to_blender(order)) {
    case EULER_ORDER_XZY:
...
static bool set_rotation_from_matrix(ClarityObjectTransform &transform,
                                     const double3x3 &rotation_matrix,
                                     const eClarityRotationOrder order,
                                     const bool use_compatible_euler)
{
  const math::Euler3Base<double> reference(double3(transform.rotation),
                                           clarity_math_rotation_order(order));
...
static double scale_compatibility_score(const double3 &scale, const ClarityObjectTransform &reference)
{
  double score = 0.0;
  for (int axis = 0; axis < 3; axis++) {
...
{
  ClarityLinearDecomposition result;
                                            ...
                                            {1.0, 1.0, -1.0}})
  {
    ...
    ClarityObjectTransform candidate = reference;
...
static void zero_special_pivots(ClarityObjectTransform &transform)
{
  for (int axis = 0; axis < 3; axis++) {
...
static void zero_rotate_axis(ClarityObjectTransform &transform)
{
                                           ...
                                           const ClarityTransformSetOptions &options)
{
  ...
  ClarityObjectTransform result = transform;
  ...
  if (!options.preserve_rotate_axis || options.policy == CLARITY_TRANSFORM_SET_BAKE_SPECIAL_CHANNELS)
  {
    zero_rotate_axis(result);
  }

  if (options.policy != CLARITY_TRANSFORM_SET_TRANSLATION_ONLY) {
    const ClarityLinearDecomposition decomposition = BKE_clarity_decompose_linear_compatible(
        double3x3(target_channel_matrix), result);
    if (!decomposition.valid) {
      return false;
    }

    if (options.policy != CLARITY_TRANSFORM_SET_SCALE_ONLY) {
      for (int axis = 0; axis < 3; axis++) {
        result.rotation[axis] = decomposition.rotation[axis];
      }
    }
    if (ELEM(options.policy,
             CLARITY_TRANSFORM_SET_CHANNELS,
             CLARITY_TRANSFORM_SET_RESET_PIVOTS,
             CLARITY_TRANSFORM_SET_BAKE_SPECIAL_CHANNELS))
    {
      for (int axis = 0; axis < 3; axis++) {
    ...
    if (options.policy != CLARITY_TRANSFORM_SET_ROTATION_ONLY) {
      for (int axis = 0; axis < 3; axis++) {
        result.scale[axis] = decomposition.scale[axis];
      }
    }

    if (!options.use_compatible_euler && options.policy != CLARITY_TRANSFORM_SET_SCALE_ONLY) {
  ...
  if (ELEM(options.policy,
           ...
           CLARITY_TRANSFORM_SET_BAKE_SPECIAL_CHANNELS))
  {
    ClarityObjectTransform without_translation = result;
    for (double &value : without_translation.translation) {
      value = 0.0;
    }
    const double3 translation = target_channel_matrix.location() -
                                BKE_clarity_transform_channel_matrix(without_translation).location();
    for (int axis = 0; axis < 3; axis++) {
...
static void offset_parent_matrix_set(ClarityObjectTransform &transform, const double4x4 &matrix)
{
...
bool BKE_object_clarity_set_dag_local_matrix(Object &object,
                                          ...
                                          const ClarityTransformSetOptions &options)
{
  if (!BKE_object_uses_clarity_transform(&object) || !matrix_is_finite(target_dag_local)) {
    return false;
  }
  ...
  if (!BKE_clarity_transform_set_channel_matrix(result, target_channel, options)) {
    return false;
  }
...
bool BKE_object_clarity_set_world_matrix(Object &object,
                                      ...
                                      const ClarityTransformSetOptions &options)
{
  if (!BKE_object_uses_clarity_transform(&object) || !matrix_is_finite(target_world)) {
    return false;
  }
  ...
  return BKE_object_clarity_set_dag_local_matrix(object, target_dag_local, options);
}

bool BKE_clarity_transform_set_rotation_order(ClarityObjectTransform &transform,
                                           const eClarityRotationOrder new_order,
                                           const bool preserve)
{
  if (new_order < CLARITY_ROT_ORDER_XYZ || new_order > CLARITY_ROT_ORDER_ZYX) {
    return false;
  }
                                        ...
                                        const bool preserve)
{
  if (preserve) {
    ...
    ClarityObjectTransform new_axis_transform = transform;
    for (int axis = 0; axis < 3; axis++) {
...
static double4x4 clarity_parent_offset_prefix(const ClarityObjectTransform &transform,
                                           const double4x4 &parent_effect_matrix)
{
...
double3 BKE_clarity_rotate_pivot_world_get(const ClarityObjectTransform &transform,
                                        const double4x4 &parent_effect_matrix)
{
  const double4x4 prefix = clarity_parent_offset_prefix(transform, parent_effect_matrix) *
...
double3 BKE_object_clarity_rotate_pivot_world_get(const Object &object,
                                               const double4x4 &parent_effect_matrix)
{
  BLI_assert(BKE_object_uses_clarity_transform(&object));
  return BKE_clarity_rotate_pivot_world_get(*object.clarity_transform, parent_effect_matrix);
}
...
{
  BLI_assert(BKE_object_uses_clarity_transform(&object));
  return BKE_clarity_scale_pivot_world_get(*object.clarity_transform, parent_effect_matrix);
}
                                           ...
                                           float r_axis[3][3])
{
  ...
  return orthonormalize_matrix(
      double3x3(clarity_parent_offset_prefix(*object.clarity_transform, parent_effect_matrix)), r_axis);
}
                                           ...
                                           float r_axis[3][3])
{
  ...
  float gimbal[3][3];
  eulO_to_gimbal_axis(
      gimbal, rotation, BKE_clarity_rotation_order_to_blender(transform.rotation_order));
