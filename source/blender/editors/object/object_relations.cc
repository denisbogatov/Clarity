...
#define INDEX_UNSET -1
  ...
  CTX_DATA_BEGIN (C, Object *, ob, selected_editable_objects) {
    if (ob != obedit) {
      const bool uses_clarity_transform = BKE_object_uses_clarity_transform(ob);
      const double4x4 clarity_world_matrix = uses_clarity_transform ? double4x4(ob->object_to_world()) :
                                                                double4x4::identity();
      ...
      if (BKE_object_parent_loop_check(par, ob)) {
        ...
        if (uses_clarity_transform) {
          BKE_object_clarity_parent_keep_transform(ob, double4x4(parent_effect), clarity_world_matrix);
        }
        else {
...
void parent_clear(Object *ob, const int type)
{
  ...
  const bool clarity_keep_transform = type == CLEAR_PARENT_KEEP_TRANSFORM &&
                                   BKE_object_uses_clarity_transform(ob);
  const double4x4 clarity_world_matrix = clarity_keep_transform ? double4x4(ob->object_to_world()) :
                                                            double4x4::identity();
  ...
  switch (type) {
    ...
    case CLEAR_PARENT_KEEP_TRANSFORM: {
      ...
      parent_clear_data(ob);
      if (clarity_keep_transform) {
        BKE_object_clarity_clear_parent_keep_transform(ob, clarity_world_matrix);
      }
      else {
                                      ...
                                      const int vert_par[3])
{
  ...
  const bool clarity_keep_transform = keep_transform && BKE_object_uses_clarity_transform(ob);
  const double4x4 clarity_world_matrix = clarity_keep_transform ? double4x4(ob->object_to_world()) :
                                                            double4x4::identity();
  ...
  if (keep_transform && !clarity_keep_transform) {
  ...
  if (BKE_object_uses_clarity_transform(ob) && partype != PAR_PATH_CONST) {
    unit_m4(ob->parentinv);
    if (clarity_keep_transform) {
      const double4x4 parent_effect_matrix(BKE_object_calc_parent(depsgraph, scene, ob));
      if (!BKE_object_clarity_parent_keep_transform(ob, parent_effect_matrix, clarity_world_matrix)) {
        BKE_report(reports,
                   RPT_WARNING,
                   "Could not preserve Clarity transform while parenting a singular matrix");
      }
    }
  }
    ...
    if (ob != par) {
      ...
      if (BKE_object_parent_loop_check(par, ob)) {
        ...
        if (keep_transform) {
          if (clarity_keep_transform) {
            const double4x4 parent_effect_matrix(BKE_object_calc_parent(depsgraph, scene, ob));
            BKE_object_clarity_parent_keep_transform(ob, parent_effect_matrix, clarity_world_matrix);
          }
          else {
