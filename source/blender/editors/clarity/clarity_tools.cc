...
struct ClarityStoredSelectionHistoryEntry {
  uintptr_t element = 0;
  char htype = 0;
};

struct ClarityStoredComponentSelection {
  Vector<uintptr_t> elements;
  Vector<ClarityStoredSelectionHistoryEntry> history;
};

struct ClarityStoredMeshSelection {
  uint64_t topology_fingerprint = 0;
  std::array<ClarityStoredComponentSelection, 3> domains;
  uintptr_t active_face = 0;
};

struct ClaritySelectionMemory {
  Map<uint32_t, ClarityStoredMeshSelection> meshes;
};

struct ClarityEditMeshBackup {
...
struct ClarityShiftTransformState {
  ...
  Vector<ClarityEditMeshBackup> edit_mesh_backups;

  ~ClarityShiftTransformState()
  {
    for (ClarityEditMeshBackup &item : edit_mesh_backups) {
      EDBM_redo_state_free(&item.backup);
    }
  }
};

static int component_mode_select_bits(const ClarityComponentMode mode)
{
  switch (mode) {
    case ClarityComponentMode::Vertex:
    case ClarityComponentMode::UV:
      return SCE_SELECT_VERTEX;
    case ClarityComponentMode::Edge:
      return SCE_SELECT_EDGE;
    case ClarityComponentMode::Face:
      return SCE_SELECT_FACE;
    case ClarityComponentMode::VertexFace:
      return SCE_SELECT_VERTEX | SCE_SELECT_FACE;
    case ClarityComponentMode::Multi:
      return SCE_SELECT_VERTEX | SCE_SELECT_EDGE | SCE_SELECT_FACE;
    case ClarityComponentMode::Object:
      return 0;
  }
...
static void component_selection_memory_prune(Main &bmain, ClaritySelectionMemory &memory)
{
...
static void component_selection_store(ClaritySelectionMemory &memory,
                                      Main &bmain,
                                      Object *object,
                                      const int select_bits)
{
  ...
  ClarityStoredMeshSelection &stored = memory.meshes.lookup_or_add_default(
  ...
  for (const int domain : IndexRange(3)) {
    ...
    ClarityStoredComponentSelection &selection = stored.domains[domain];
...
static void component_selection_restore(ClaritySelectionMemory &memory,
                                        Main &bmain,
                                        Object *object,
                                        const int select_bits)
{
  ...
  ClarityStoredMeshSelection *stored = memory.meshes.lookup_ptr(mesh_uid);
  if (stored != nullptr &&
  ...
  if (stored != nullptr) {
    for (const int domain : IndexRange(3)) {
      ...
      for (const ClarityStoredSelectionHistoryEntry &item : stored->domains[domain].history) {
                                                     ...
                                                     const ClarityWindowRuntime &runtime)
{
  ...
  if (runtime.component_mode != ClarityComponentMode::Object &&
  ...
  if (select_mode == (SCE_SELECT_VERTEX | SCE_SELECT_EDGE | SCE_SELECT_FACE)) {
    return ClarityComponentMode::Multi;
  }
  if (select_mode == (SCE_SELECT_VERTEX | SCE_SELECT_FACE)) {
    return ClarityComponentMode::VertexFace;
  }
...
static bool component_mode_set(bContext *C,
                               ...
                               const ClarityComponentMode requested_mode)
{
  ClarityComponentMode current_mode = component_mode_from_context(C, runtime);
  const ClarityComponentMode target_mode = requested_mode;
  if (current_mode == target_mode) {
    runtime.component_mode = target_mode;
    if (target_mode != ClarityComponentMode::Object) {
  ...
  if (!runtime.selection_memory) {
    runtime.selection_memory = std::make_shared<ClaritySelectionMemory>();
  }

  if (current_mode != ClarityComponentMode::Object &&
  ...
  if (target_mode == ClarityComponentMode::Object) {
    ...
    runtime.component_mode = ClarityComponentMode::Object;
    return true;
  }
...
static const EnumPropertyItem clarity_component_mode_items[] = {
    {int(ClarityComponentMode::Object), "OBJECT", 0, "Object", "Select whole objects"},
    {int(ClarityComponentMode::Vertex), "VERTEX", 0, "Vertex", "Select mesh vertices"},
    {int(ClarityComponentMode::Edge), "EDGE", 0, "Edge", "Select mesh edges"},
    {int(ClarityComponentMode::Face), "FACE", 0, "Face", "Select mesh faces"},
    {int(ClarityComponentMode::UV),
     "UV",
     0,
     "UV (Experimental)",
     "Experimental compatibility mode; currently uses mesh vertices, not per-corner UVs"},
    {int(ClarityComponentMode::VertexFace),
     "VERTEX_FACE",
     0,
     "Vertex Face (Experimental)",
     "Experimental compatibility mode; currently combines vertex and face selection"},
    {int(ClarityComponentMode::Multi),
     "MULTI",
     0,
     "Multi-Component",
...
static const EnumPropertyItem clarity_camera_based_selection_items[] = {
...
static wmOperatorStatus clarity_component_mode_set_exec(bContext *C, wmOperator *op)
{
  ...
  const ClarityComponentMode mode = ClarityComponentMode(RNA_enum_get(op->ptr, "mode"));
  return component_mode_set(C, *runtime, mode) ? OPERATOR_FINISHED : OPERATOR_CANCELLED;
}

static void CLARITY_OT_component_mode_set(wmOperatorType *ot)
{
  ot->name = "Clarity Component Mode";
  ot->description = "Switch object or polygon component mode while preserving each component set";
  ot->idname = "CLARITY_OT_component_mode_set";
  ot->exec = clarity_component_mode_set_exec;
  ot->poll = ED_operator_view3d_active;
  ot->flag = OPTYPE_UNDO;
  RNA_def_enum(ot->srna,
               "mode",
               clarity_component_mode_items,
               int(ClarityComponentMode::Vertex),
               "Mode",
               "");
}

static wmOperatorStatus clarity_selection_settings_set_exec(bContext *C, wmOperator *op)
{
  ClarityWindowRuntime *runtime = runtime_ensure(C);
  if (runtime == nullptr) {
    return OPERATOR_CANCELLED;
  }
  ClaritySelectionSettings &settings = runtime->selection_settings;
  ...
  if (RNA_struct_property_is_set(op->ptr, "camera_based_selection")) {
    settings.camera_based_selection = ClarityCameraBasedSelection(
...
static void CLARITY_OT_selection_settings_set(wmOperatorType *ot)
{
  ot->name = "Clarity Selection Settings";
  ot->description = "Set per-window Clarity selection behavior";
  ...
  RNA_def_float(ot->srna,
                ...
                100.0f,
                "Click Box Size",
                "Clarity selection hit radius in pixels",
                0.0f,
                20.0f);
  RNA_def_float(ot->srna,
  ...
  RNA_def_enum(ot->srna,
               ...
               clarity_camera_based_selection_items,
               int(ClarityCameraBasedSelection::Auto),
...
static wmOperatorStatus clarity_pivot_pin_toggle_exec(bContext *C, wmOperator * /*op*/)
{
  ClarityWindowRuntime *runtime = runtime_ensure(C);
...
static void CLARITY_OT_pivot_pin_toggle(wmOperatorType *ot)
{
  ot->name = "Clarity Pin Pivot";
...
static const EnumPropertyItem clarity_pivot_reset_action_items[] = {
...
static const EnumPropertyItem clarity_pivot_bake_mode_items[] = {
    {CLARITY_PIVOT_BAKE_POSITION, "POSITION", 0, "Position", "Bake the custom pivot position"},
    {CLARITY_PIVOT_BAKE_ORIENTATION,
     "ORIENTATION",
     0,
     "Orientation",
     "Bake the custom pivot orientation"},
    {CLARITY_PIVOT_BAKE_BOTH, "BOTH", 0, "Both", "Bake custom position and orientation"},
    {0, nullptr, 0, nullptr, nullptr},
};

static wmOperatorStatus clarity_pivot_edit_toggle_exec(bContext *C, wmOperator * /*op*/)
{
  ClarityWindowRuntime *runtime = runtime_ensure(C);
...
static void CLARITY_OT_pivot_edit_toggle(wmOperatorType *ot)
{
  ot->name = "Clarity Edit Pivot";
  ot->description = "Enter or leave persistent Clarity Edit Pivot mode";
...
{
  ClarityPivotToolSettings settings;
  ED_clarity_pivot_tool_settings_get(C, settings);
  ...
  if (RNA_struct_property_is_set(op->ptr, "reset_mode")) {
    settings.reset_mode = eClarityPivotResetMode(RNA_enum_get(op->ptr, "reset_mode"));
  }
  return ED_clarity_pivot_tool_settings_set(C, settings) ? OPERATOR_FINISHED : OPERATOR_CANCELLED;
}

static void CLARITY_OT_pivot_settings_set(wmOperatorType *ot)
{
  ot->name = "Clarity Pivot Settings";
  ot->description = "Set Clarity manipulator-pivot behavior";
  ...
  RNA_def_enum(ot->srna,
               "reset_mode",
               clarity_pivot_reset_mode_items,
               CLARITY_PIVOT_RESET_CENTER,
               "Reset Mode",
               "");
}

static wmOperatorStatus clarity_pivot_reset_exec(bContext *C, wmOperator *op)
{
  ED_clarity_pivot_undo_begin(C);
  ClarityPivotToolSettings settings;
  ED_clarity_pivot_tool_settings_get(C, settings);
  const eClarityPivotResetMode mode = RNA_struct_property_is_set(op->ptr, "mode") ?
                                       eClarityPivotResetMode(RNA_enum_get(op->ptr, "mode")) :
                                       settings.reset_mode;
  switch (RNA_enum_get(op->ptr, "action")) {
    case 0:
      ED_clarity_pivot_reset_position(C, mode);
      break;
    case 1:
      ED_clarity_pivot_reset_orientation(C);
      break;
    case 2:
      ED_clarity_pivot_reset_all(C, mode);
      break;
    default:
      return OPERATOR_CANCELLED;
  }
...
static void CLARITY_OT_pivot_reset(wmOperatorType *ot)
{
  ot->name = "Reset Clarity Pivot";
  ot->description = "Reset Clarity pivot position, orientation, or both";
  ot->idname = "CLARITY_OT_pivot_reset";
  ot->exec = clarity_pivot_reset_exec;
  ot->poll = ED_operator_view3d_active;
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;
  RNA_def_enum(ot->srna, "action", clarity_pivot_reset_action_items, 2, "Reset", "");
  RNA_def_enum(
      ot->srna, "mode", clarity_pivot_reset_mode_items, CLARITY_PIVOT_RESET_CENTER, "Mode", "");
}

static wmOperatorStatus clarity_pivot_bake_exec(bContext *C, wmOperator *op)
{
  ED_clarity_pivot_undo_begin(C);
  if (!ED_clarity_pivot_bake(C, eClarityPivotBakeMode(RNA_enum_get(op->ptr, "mode")))) {
...
static bool clarity_pivot_bake_poll(bContext *C)
{
...
static void CLARITY_OT_pivot_bake(wmOperatorType *ot)
{
  ot->name = "Bake Clarity Pivot";
  ot->description = "Bake Clarity manipulator pivot state into the object and its geometry";
  ot->idname = "CLARITY_OT_pivot_bake";
  ot->exec = clarity_pivot_bake_exec;
  ot->poll = clarity_pivot_bake_poll;
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;
  RNA_def_enum(ot->srna,
               "mode",
               clarity_pivot_bake_mode_items,
...
void register_tool_operators()
{
  ...
  WM_operatortype_append(CLARITY_OT_pivot_reset);
  WM_operatortype_append(CLARITY_OT_pivot_bake);
...
#define REGISTER_MAYA_OPERATOR_COMPATIBILITY(name) \
  ...
  WM_operatortype_append(REGISTER_MAYA_OPERATOR_COMPATIBILITY(pivot_reset));
  WM_operatortype_append(REGISTER_MAYA_OPERATOR_COMPATIBILITY(pivot_bake));
#undef REGISTER_MAYA_OPERATOR_COMPATIBILITY
}

static wmOperatorStatus component_mode_operator_call(bContext *C, const ClarityComponentMode mode)
{
  wmOperatorType *ot = WM_operatortype_find("CLARITY_OT_component_mode_set", true);
...
bool camera_based_selection_use_depth(const ClarityCameraBasedSelection mode,
                                      ...
                                      const bool is_xray)
{
  ...
  switch (mode) {
    case ClarityCameraBasedSelection::Off:
      return false;
    case ClarityCameraBasedSelection::On:
      return true;
    case ClarityCameraBasedSelection::Auto:
      return is_shaded;
  }
  BLI_assert_unreachable();
  return false;
...
static wmOperatorStatus select_marquee_call(bContext *C, const ClarityInputAction &action)
{
  ...
  /* Every Clarity marquee operation uses the projected face area, not its center: replace, toggle,
 ...
 * the loop of an upright edge is that one edge. Clarity's double click on the same edge selects all
 ...
 */
static ClarityEdgePathPick edge_path_pick_from_event(bContext *C,
                                                  const wmEvent &event,
                                                  BMEdge *source)
{
  ClarityEdgePathPick result;
 ...
 * made whatever is under the pointer the active face. Clarity reads the pair - the face that was
...
static void topology_anchor_store(bContext *C, ClarityWindowRuntime &runtime)
{
...
static BMElem *topology_anchor_elem_get(bContext *C, const ClarityWindowRuntime &runtime, BMesh *bm)
{
  const Object *object = CTX_data_edit_object(C);
...
static BMFace *topology_anchor_face_get(bContext *C, const ClarityWindowRuntime &runtime, BMesh *bm)
{
...
static ClarityAdjacentFacePick adjacent_face_pick_from_event(bContext *C,
                                                          const ClarityWindowRuntime &runtime,
                                                          const wmEvent &event)
{
  ClarityAdjacentFacePick result;
  ViewContext vc = em_setup_viewcontext(C);
  copy_v2_v2_int(vc.mval, event.mval);
  view3d_operator_needs_gpu(C);
...
bool selection_action_is_reserved_for_marquee(const ClarityInputAction &action)
{
   ...
   *
   * A double click of the same chord is a different gesture: `Ctrl Shift` is how Clarity says "add to
  ...
  return action.id == ClarityActionID::SelectAddMarquee;
}

ClarityTopologySelectOp topology_select_op_from_action(const ClarityInputAction &action)
{
  if (action.ctrl && action.shift) {
    return ClarityTopologySelectOp::Add;
  }
  if (action.ctrl) {
    return ClarityTopologySelectOp::Subtract;
  }
  if (action.shift) {
    return ClarityTopologySelectOp::Toggle;
  }
  return ClarityTopologySelectOp::Replace;
}

/**
 * Whether a previously selected component can act as the start of a path.
 *
 * Clarity takes the last selected component as the beginning of the gesture. Without one there is
 ...
 */
static bool has_path_source_component(bContext *C, const ClarityWindowRuntime &runtime)
{
  Object *object = CTX_data_edit_object(C);
  BMEditMesh *em = object != nullptr ? BKE_editmesh_from_object(object) : nullptr;
  if (em == nullptr || em->bm == nullptr) {
 ...
 *
 * Clarity's `polySelectConstraint`: a standing rule about what a selection is allowed to contain.
...
static bool selection_filter_by_edge(bContext *C,
                                     const ClarityEdgePredicate predicate,
...
static bool selection_constraint_apply(bContext *C)
{
  ...
  const ClaritySelectionConstraint constraint = runtime->selection_settings.selection_constraint;
  if (constraint == ClaritySelectionConstraint::Off || !is_mesh_component_context(C)) {
    return false;
  }
...
static void topology_select_op_properties_set(PointerRNA *ptr, const ClarityTopologySelectOp op)
{
  RNA_boolean_set(ptr, "extend", op == ClarityTopologySelectOp::Add);
  RNA_boolean_set(ptr, "deselect", op == ClarityTopologySelectOp::Subtract);
  RNA_boolean_set(ptr, "toggle", op == ClarityTopologySelectOp::Toggle);
}

static wmOperatorStatus select_loop_call(bContext *C,
                                         const ClarityInputAction &action,
                                         const ClarityTopologySelectOp op)
{
   ...
   * again. Clarity keeps this gesture additive, which is also what #select_adjacent_face_loop_call
   * does for the face loop. */
  const ClarityTopologySelectOp loop_op = op == ClarityTopologySelectOp::Toggle ?
                                           ClarityTopologySelectOp::Add :
                                           op;

  /* On a box the loop of an upright edge is that edge alone, because every corner is a pole and
   * Blender stops there. Clarity answers the same double click with all four uprights, which is the
...
static wmOperatorStatus select_face_shell_call(bContext *C)
{
  ...
  PointerRNA ptr = WM_operator_properties_create_ptr(ot);
  /* A Clarity shell is delimited only by disconnected topology, not by seams or other edge marks. */
...
static wmOperatorStatus select_adjacent_face_loop_call(bContext *C,
                                                       const ClarityAdjacentFacePick &pick)
{
  const Main *bmain = CTX_data_main(C);
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
   ...
   * Toggle would immediately remove the requested loop, while Clarity keeps this gesture additive. */
  topology_select_op_properties_set(&ptr, ClarityTopologySelectOp::Add);
                                         ...
                                         const ClarityTopologySelectOp op)
{
  ...
  ClarityEdgePathPick edge_pick = anchor->head.htype == BM_EDGE ?
      ...
      edges_are_opposite_in_quad(edge_pick.source, edge_pick.destination))
  {
    ...
    topology_select_op_properties_set(&ptr,
                                      op == ClarityTopologySelectOp::Toggle ?
                                          ClarityTopologySelectOp::Add :
                                          op);
    const wmOperatorStatus status = WM_operator_name_call_ptr(
        C, ot, wm::OpCallContext::InvokeDefault, &ptr, action.source_event);
   ...
   * on "all selected", which is exactly the guaranteed remove and the guaranteed add Clarity offers. The
  ...
  const bool needs_second_call = ELEM(op, ClarityTopologySelectOp::Add, ClarityTopologySelectOp::Subtract);
  ...
  if (needs_second_call && (status & OPERATOR_FINISHED)) {
    ...
    const bool toggle_went_the_wrong_way = op == ClarityTopologySelectOp::Add ?
  ...
  if (needs_second_call && wm != nullptr) {
    wm->op_undo_depth--;
    if (status & OPERATOR_FINISHED) {
      ED_undo_push(C, op == ClarityTopologySelectOp::Add ? "Add Select Path" : "Remove Select Path");
    }
  }
  WM_operator_properties_free(&ptr);
                                            ...
                                            const ClarityInputAction &action)
{
  ...
  if (!is_window_region &&
      ELEM(action.id,
           ...
           ClarityActionID::SelectAddMarquee,
           ClarityActionID::SelectTopology))
  {
    return ClarityDispatchResult::PassThrough;
  }

  ClarityComponentMode mode = ClarityComponentMode::Object;
  bool is_mode_action = true;
  switch (action.id) {
    case ClarityActionID::ToggleObjectComponent:
      mode = component_mode_from_context(C, runtime) == ClarityComponentMode::Object ?
                 runtime.last_component_mode :
                 ClarityComponentMode::Object;
      break;
    case ClarityActionID::ComponentVertex:
      mode = ClarityComponentMode::Vertex;
      break;
    case ClarityActionID::ComponentEdge:
      mode = ClarityComponentMode::Edge;
      break;
    case ClarityActionID::ComponentFace:
      mode = ClarityComponentMode::Face;
      break;
    case ClarityActionID::ComponentUV:
      mode = ClarityComponentMode::UV;
      break;
    case ClarityActionID::ComponentVertexFace:
      mode = ClarityComponentMode::VertexFace;
      break;
    case ClarityActionID::ComponentMulti:
      mode = ClarityComponentMode::Multi;
      break;
    default:
  ...
  if (is_mode_action) {
    ...
    return (status & OPERATOR_FINISHED) ? ClarityDispatchResult::Handled :
                                         ClarityDispatchResult::PassThrough;
  }
  ...
  {
    eSelectOp select_op = SEL_OP_SET;
    if (action.id == ClarityActionID::SelectAdd) {
      select_op = SEL_OP_ADD;
    }
    else if (action.id == ClarityActionID::SelectRemove) {
      select_op = SEL_OP_SUB;
    }
    else if (action.id == ClarityActionID::SelectToggle) {
      select_op = SEL_OP_XOR;
    }
  ...
  if (action.id == ClarityActionID::SelectTopology && is_mesh_component_context(C)) {
    ...
    const bool is_face_mode = component_mode_from_context(C, runtime) == ClarityComponentMode::Face;
    const bool use_face_shell = op == ClarityTopologySelectOp::Replace && is_face_mode;
    ClarityAdjacentFacePick face_loop_pick;
    /* Every additive form of the gesture, not just `Shift`: `Ctrl Shift` is how Clarity says "add to
    ...
    if (is_face_mode && ELEM(op, ClarityTopologySelectOp::Toggle, ClarityTopologySelectOp::Add)) {
 ...
 */
static bool is_drag_threshold_crossing(const ClarityWindowRuntime &runtime,
                                       const ClarityInputAction &action,
                                       const bContext *C,
                                       const wmEventType button)
{
  ...
  return !runtime.transform_active && action.id == ClarityActionID::PointerMove &&
         action.phase == ClarityActionPhase::Update && !action.alt && event != nullptr &&
                                    ...
                                    const ClarityInputAction &action)
{
  ...
  /* Clarity reads the modifiers of the press that started the drag, not of the motion that happens to
   * cross the threshold. */
  ClarityInputAction drag = action;
                                   ...
                                   const ClarityInputAction &action)
{
  ...
  const bool edit_pivot = runtime.pivot_edit.target != ClarityPivotEditTarget::None;
  PropertyRNA *clarity_pivot_transform = RNA_struct_find_property(&ptr, "clarity_pivot_transform");
  if (edit_pivot && clarity_pivot_transform == nullptr) {
    /* Edit Pivot must never fall back to transforming the object. */
    WM_operator_properties_free(&ptr);
    return true;
  }
  if (clarity_pivot_transform != nullptr) {
    RNA_property_boolean_set(&ptr, clarity_pivot_transform, edit_pivot);
  }
  ...
  if (runtime.tool.active == ClarityToolID::Scale) {
    ...
    if (display_matrix != nullptr) {
      ...
      RNA_boolean_set(&ptr, "use_clarity_scale_behavior", true);
    }
  }
...
static void shift_transform_edit_backups_free(ClarityShiftTransformState &state)
{
  for (ClarityEditMeshBackup &item : state.edit_mesh_backups) {
    EDBM_redo_state_free(&item.backup);
  }
...
static bool shift_transform_object_duplicate(bContext *C,
                                             ClarityWindowRuntime &runtime,
                                             ClarityShiftTransformState &state)
{
...
/**
 * Clarity's Edge Slide transform constraint, served by Blender's own slide transforms.
 *
...
{
  if (op == nullptr || event == nullptr || !ED_clarity_interaction_enabled(C)) {
    return false;
  }
  if (ED_clarity_transform_constraint_get(C) != ClarityTransformConstraint::Edge) {
    return false;
  }
   ...
   * beside it. Clarity makes the same split. */
...
bool shift_transform_prepare(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  std::shared_ptr<ClarityShiftTransformState> state = std::make_shared<ClarityShiftTransformState>();
  ...
  if (edit_object != nullptr && edit_object->type == OB_MESH) {
    if (runtime->selection_settings.shift_extrude) {
      state->kind = ClarityShiftTransformState::Kind::ComponentExtrude;
    ...
    if (runtime->selection_settings.shift_duplicate) {
      state->kind = ClarityShiftTransformState::Kind::ObjectDuplicate;
...
static void shift_transform_object_restore(bContext *C, ClarityShiftTransformState &state)
{
  Main *bmain = CTX_data_main(C);
...
static void shift_transform_component_restore(ClarityShiftTransformState &state)
{
  for (ClarityEditMeshBackup &item : state.edit_mesh_backups) {
...
void shift_transform_end(bContext *C, ClarityWindowRuntime &runtime, const bool cancelled)
{
  if (!runtime.shift_transform) {
    return;
  }

  ClarityShiftTransformState &state = *runtime.shift_transform;
  if (cancelled) {
    if (state.kind == ClarityShiftTransformState::Kind::ObjectDuplicate) {
  ...
  else if (state.kind == ClarityShiftTransformState::Kind::ComponentExtrude) {
    shift_transform_edit_backups_free(state);
  }
...
{
  return ed::clarity::shift_transform_prepare(C, op, event);
}

bool ED_clarity_transform_slide_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  return ed::clarity::transform_slide_invoke(C, op, event);
}
